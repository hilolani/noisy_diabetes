...
from . import data  # noisy_diabetes/data/
...
def load_noisy_diabetes(return_X_y=False, as_frame=False, scaled=True):
    ...
    with files(data).joinpath("noisy_diabetes_data.csv").open("r", encoding="utf-8") as f:
        reader = csv.reader(f)
...
def load_total_noisy_diabetes(return_X_y=False, as_frame=False, scaled=True):
    ...
    with files(data).joinpath("total_noisy_diabetes_data.csv").open("r", encoding="utf-8") as f:
        reader = csv.reader(f)
        l = [row for row in reader]
    f.close()
    del(l[0])
